...
df_ideb_full %>% 
  ...
  bind_rows(df_ideb_sp %>% mutate(abrangencia = 'Municípios Paulistas')) %>%
  ggplot() +
  geom_density(aes(x = nota, fill = nivel), alpha = 0.5, position = 'identity') +
...
ggsave('densidade_ideb_brasil_vs_sp.png', width = 7, height = 7)
