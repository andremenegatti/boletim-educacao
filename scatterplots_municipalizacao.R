...
# Scatterplot - Municípios ----------------------------------------------------
df_iniciais %>% 
  ggplot(aes(x = municipalizacao * 100, y = ideb)) +
  geom_point(alpha = 0.2, shape = 1) +
  geom_smooth(method = 'lm', col = 'darkred',
              fill = 'lightgray', se = TRUE) +
  cagedExplorer::custom_theme() +
  scale_x_continuous(labels = function(x) paste0(x, '%')) +
  scale_y_continuous(labels = 
                       function(x) formatC(x, big.mark = '.',
...
# ggsave('plots/scatterplot_ideb_vs_municipalizacao_municipios.png', height = 6, width = 6)
